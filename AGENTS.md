...
- `browser-extension/lib/vault.js` `normalizeDomain` strips every subdomain down to the registrable domain before saving or looking up entries (e.g. `member.lazada.co.th` → `lazada.co.th`). Suffixes are resolved from the `PUBLIC_SUFFIXES` list first; when nothing matches, a compound suffix is inferred from a generic label plus a 2-character country TLD (`com.ve`, `ac.in`), which is why `banesco.com.ve` and `otrobanco.com.ve` stay separate while `mail.bbc.com` still collapses to `bbc.com`. Both halves are load-bearing: without the list, multi-tenant hosts like `github.io` would share one entry; without the fallback, every host under an unlisted compound suffix would collapse onto the bare suffix and share credentials across unrelated sites. Do not revert this to a plain `url.hostname` return either — it would cause the same secret to appear as a different entry on different subdomains.
